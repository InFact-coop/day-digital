...
router.route('/video-upload').post((req, res, next) => {
  fs.rename(req.file.path, `${req.file.path}.mp4`, err => {
    // cloudinary documentation is wrong here, for uploader.upload the callback
    // is the second argument and the options are the third
    // the callback also has the arguments the wrong way round with the
    // 'result' going first and the 'err' last
